...
```{r extra pika occupancy sites close to locations of interest, include = T}
...
pika_habsite_grps <- list(arik_pika = c(102, 038, 22),
                          ...
                          gl5out_pika = c(086, 069, 005, 053),
                          gl4_pika = c(117, 002, 009, 073),
                          d1_pika = c(058, 030, 003),
                          gl3_pika = c(085, 043, 059), # 085 is high elevation (up in the talus) compared to 43 and 59 (closer to lake)
                          gl2_pika = c(027, 055),
                          gl1_pika = c(031, 095, 023),
                          sdl_pika = c(056, 060),
                          glv_overlook_pika = c(091, 014, 186, 062, 079, 015),
                          below_sensortvan_pika = c(035, 016),
...
left_join(pika_habsite_lut, pika_habocc_sites, by = c("pikahab_plot" = "plot")) %>%
  ...
  #geom_text(aes(label = gsub("NWT-", "", pikahab_plot))) +
  facet_wrap(~pikahab_grp) +
...
pika_habocc_dailies <- mutate(pika_haboccT, pikanum = parse_number(gsub("NWT-", "", plot))) %>%
  left_join(mutate(pika_habsite_lut, pikanum = parse_number(gsub("NWT-", "", pikahab_plot)))) %>%
  subset(!is.na(pikahab_grp))
...
pika_habocc_dailies <- subset(pika_habocc_dailies, !(fecha_flag & flag_nobs)) %>%
...
```{r saddle grid snow depth, include = T}
...
To deal with patchy temporal overlap of datasets (to allow for testing for significance on correlation matrix), I've subsetted the compiled data into three periods:
...
Currently there are not that many variables in the "late" period, so I only show correlation plots (no heatmaps--not interesting). I also realized afterwards I am leaving out year 2015 from analysis, so in future revisions I can be sure correlations are run including as many year as possible. For today, the three-time period grouping I did was a workaround.
...
```{r summarize weekly, include = T, fig.height=8, fig.width=8}
...
```{r weekly correlations by season, include = T, fig.height= 8, fig.width=8}
winter <- c(12,1,2)
fall <- c(9,10,11)
spring <- c(3,4,5)
summer <- c(6,7,8)
