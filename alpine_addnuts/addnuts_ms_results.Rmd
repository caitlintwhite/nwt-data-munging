...
Out of curiosity, and acknowledging differences in sampling methods in certain years (e.g. top hits only in 1997, 50-point subset in 2005) is there a treatment effect and grass and forb hits (not relatived cover) within year?
...
Disimilar to Saddle trends, nothing is significant at the NutNet site. 
...
Out of curiosity, I repeated this for Saddle for years where biodiversity or biomass data are available:

```{r sdl biodiversity}
sdl_biodiv_means <- #subset(sdl_coarse_means, yr %in% unique(sdl_coarse_tall$yr[sdl_coarse_tall$met == "H" & !is.na(sdl_coarse_tall$val)])) %>%
  sdl_coarse_means %>%
...
Considering the same plots analyzed above, what can we learn from multivariate analyses about shifts in forb and grass relative abundances by treatment overtime at each site?
...
ggplot(alpplotmds, aes(MDS1, MDS2)) +
  ...
  geom_polygon(data=alpells, aes(x=MDS1, y=MDS2,fill=trt), alpha = 0.5) +
