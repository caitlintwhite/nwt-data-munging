...
pcfig_wspp <- ggplot() +
  ...
  geom_text(data = subset(sppscores, PC1 >0 & PC2 <= 0 & grepl("Poly", Latin.name)), aes(PC1, PC2, label = str_wrap(Latin.name, width = 10)), size = plottext_pca, fontface = "italic", color = "black", lineheight = 0.75, vjust = 1.3, nudge_x = -0.1) +
  ...
  ## spp on neg x, pos y
  geom_text(data = subset(sppscores, PC1 <= 0 & PC2 > 0 & !grepl("Festuca|Kobre", Latin.name)), aes(PC1, PC2, label = str_wrap(Latin.name, width = 10)), size = plottext_pca, fontface = "italic", color = "black", lineheight = 0.75, nudge_x = -0.05, hjust = 1) +
  ## plot Festuca separately (-,+)
   geom_text(data = subset(sppscores, PC1 <= 0 & PC2 > 0 & grepl("Festuca", Latin.name)), aes(PC1, PC2, label = str_wrap(Latin.name, width = 10)), size = plottext_pca, fontface = "italic", color = "black", lineheight = 0.75, nudge_x = 0.1, vjust = -0.3) +
  ## plot Festuca separately (-,+)
   geom_text(data = subset(sppscores, PC1 <= 0 & PC2 > 0 & grepl("Kobre", Latin.name)), aes(PC1, PC2, label = str_wrap(Latin.name, width = 10)), size = plottext_pca, fontface = "italic", color = "black", lineheight = 0.75, vjust = -0.3, hjust = 1) +
  ...
  geom_text(data = subset(sppscores, PC1 <= 0 & PC2 < 0 & !grepl("Silene|Lloyd|Tetra", Latin.name)), aes(PC1, PC2, label = str_wrap(Latin.name, width = 10)), size = plottext_pca, fontface = "italic", color = "black", lineheight = 0.75, vjust = 1.3) +
    ...
    geom_text(data = subset(sppscores, PC1 <= 0 & PC2 < 0 & grepl("Lloy|Tetr", Latin.name)), aes(PC1, PC2, label = str_wrap(Latin.name, width = 10)), size = plottext_pca, fontface = "italic", color = "black", lineheight = 0.75, hjust = 1, nudge_x = -0.05) +
  ...
  geom_segment(aes(x=-0.1,xend=-1.5,y=(min(sppscores$PC2)-0.18),yend=(min(sppscores$PC2)-0.18)),
               arrow = arrow(length = unit(0.25, "cm")),colour="grey40", alpha = 0.4, lwd = 0.75) +
  geom_text(aes(2, (min(sppscores$PC2)-0.08), label = "Resource\nacquisitive"), col = "grey20", size = plottext_pca, hjust = 1, vjust = 0, lineheight = 0.75) +
  geom_text(aes(-1.5, (min(sppscores$PC2)-0.08), label = "Resource\nconservative"), col = "grey20", size = plottext_pca, hjust = 0, vjust = 0, lineheight = 0.75) +
  #coord_fixed() +
  theme(panel.grid = element_blank()) +
  j_theme +
  scale_color_manual(values = traitcols, guide = FALSE)
...
nnnmds <- ggplot(subset(alpplotmds, site == "nutnet" & iter == "main"), aes(MDS1, MDS2)) +
  ...
  geom_point(data = subset(alpsppmds, site == "nutnet" & iter == "main"), aes(MDS1, MDS2, shape = resource_grp, col = resource_grp), alpha = 0.75) +
