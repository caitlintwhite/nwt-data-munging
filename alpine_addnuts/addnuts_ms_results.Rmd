...
for(i in alpstress$tracker){
  alpstress[alpstress$tracker == i, c("plotx_hi", "plotx_lo", "ploty_hi", "ploty_lo")] <- c(
    max(alpsppmds$MDS1[alpsppmds$tracker == i]),
    min(alpsppmds$MDS1[alpsppmds$tracker == i]),
    max(alpsppmds$MDS2[alpsppmds$tracker == i]),
    min(alpsppmds$MDS2[alpsppmds$tracker == i]))
}
```
...
sdlnmds + geom_text(data = subset(alpstress, site == "sdl"), aes(plotx_hi, ploty_hi, label = paste("stress:", round(stress,3))), size = plottext, hjust = 1)
```
