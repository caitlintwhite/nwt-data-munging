...
kable(dplyr::select(sdl_coarse_means, site, yr, trt, grp, meancov, secov) %>%
        filter(grp != "Geum_rel") %>%
        rename(fxnlgrp = grp) %>%
...
kable(anova_df, label = "Table S2. ANOVA results for saddle, effects of treatment and functional group (forb or grass) on mean relative cover. Separate ANOVAs were run by year, n=4 per treatment, per functional group.")
```
