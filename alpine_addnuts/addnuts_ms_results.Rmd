...
# visualize
sdl_meancov_fig <- ggplot(data= subset(sdl_coarse_means, grp  %in% c("Forb_rel", "Grass_rel")), aes(trt, meanval, fill = grp)) +
...
# write out tif
ggsave("alpine_addnuts/figures/journal_figs/sdl_meancov.tiff", sdl_meancov_fig, dpi = 320,
       width = 6, height = 4, units = "in")
# write out as pdf
ggsave("alpine_addnuts/figures/journal_figs/sdl_meancov.pdf", sdl_meancov_fig, dpi = 320,
       width = 6, height = 4, units = "in")
...
# visualize
nn_relcov_fig <- ggplot(data= subset(nn_fg_means, met %in% c("Grass_rel", "Forb_rel")), aes(trt2, meancov, fill = met)) +
...
# save tiff
ggsave("alpine_addnuts/figures/journal_figs/nn_meancov.tiff", nn_relcov_fig, dpi= 320,
       width = 4, height = 2.2, units = "in")
# save pdf
ggsave("alpine_addnuts/figures/journal_figs/nn_meancov.pdf", nn_relcov_fig, dpi= 320,
       width = 4, height = 2.2, units = "in")
...
alpenv <- mutate(alpenv, signif = ifelse(pval <= 0.001, "***", ifelse(pval <= 0.01, "**", ifelse(pval <= 0.05, "*", ifelse(pval <= 0.1, ".", "n.s.")))),
                 label_x = ifelse(NMDS1 >= 0, NMDS1+0.03, NMDS1-0.03),
                 label_y = ifelse(NMDS2 >= 0, NMDS2+0.03, NMDS2-0.03))
...
sdlnmds <- ggplot(subset(alpplotmds, site == "sdl" & iter == "main"), aes(MDS1, MDS2)) +
  ...
  geom_point(data = subset(alpsppmds, site == "sdl" & iter == "main"), aes(MDS1, MDS2, shape = resource_grp, col = resource_grp), alpha = 0.8) +
...
if(any(alpenv$pval[alpenv$site == "sdl" & alpenv$grp == "vectors"] <= 0.05)){
  sdlnmds <- sdlnmds + geom_segment(data=subset(alpenv, grp == "vectors" & site == "sdl" & pval <= 0.05),aes(x=0,xend=NMDS1,y=0,yend=NMDS2),
    ...
    # for arrows pointing right
    geom_text(data=subset(alpenv, grp == "vectors" & site == "sdl" & pval <= 0.05 & label_x > 0),aes(x=label_x,y=label_y, label = paste0(signif,"F:G")), col = "black", size = plottext, fontface = "italic", hjust = 0) +
    # for arrows pointing left and down
    geom_text(data=subset(alpenv, grp == "vectors" & site == "sdl" & pval <= 0.05 & label_x <= 0),aes(x=label_x,y=label_y, label = paste0(signif,"F:G")), col = "black", size = plottext, fontface = "italic", vjust = 1)
}

# annotate stress and trt signif
## > to annotate "stress:" paste("Stress:", round(stress,3)))
sdlnmds <- sdlnmds + geom_text(data = subset(alpstress, site == "sdl"), aes(plotx_hi, ploty_hi, label = round(stress,3)), size = plottext, hjust = 1) +
...
# write out as tiff
ggsave("alpine_addnuts/figures/journal_figs/sdl_nmds.tiff", sdlnmds, dpi = 320,
       width = 6, height = 4)
# write out as pdf
ggsave("alpine_addnuts/figures/journal_figs/sdl_nmds.pdf", sdlnmds, dpi = 320,
       width = 6, height = 4)
```
...
nnnmds <- ggplot(subset(alpplotmds, site == "nutnet" & iter == "main"), aes(MDS1, MDS2)) +
  ...
  geom_point(data = subset(alpsppmds, site == "nutnet" & iter == "main"), aes(MDS1, MDS2, shape = resource_grp, col = resource_grp), alpha = 0.8) +
  scale_fill_manual(name = NULL, values = trtcols) +
  scale_color_manual(name = NULL, values = traitcols) +
  scale_shape_manual(name = NULL, values = c(19, 19, 21)) +
  j_theme +
...
if(any(with(alpenv, pval[site == "nutnet" & grepl("main", tracker) & grp == "vectors"]) <= 0.05)){
  nnnmds <- nnnmds + geom_segment(data=subset(alpenv, grp == "vectors" & grepl("main", tracker) & site == "nutnet" & pval <= 0.05),aes(x=0,xend=NMDS1,y=0,yend=NMDS2),
                                  arrow = arrow(length = unit(0.25, "cm")),colour="black") + 
    # add label to envfit arrow
    # for arrows pointing left and up
    geom_text(data=subset(alpenv, grp == "vectors" & site == "nutnet" & grepl("main", tracker) & pval <= 0.05),aes(x=label_x,y=label_y, label = paste0(signif,"F:G")), col = "black", size = plottext, fontface = "italic", hjust = 0.9, vjust = 0)
}

# annotate stress and trt signif
nnnmds <- nnnmds + geom_text(data = subset(alpstress, site == "nutnet" & grepl("main", tracker)), aes(plotx_hi, ploty_hi, label = round(stress,3)), size = plottext, hjust = 1) +
  geom_text(data = subset(alpstress, site == "nutnet" & grepl("main", tracker)), aes(plotx_hi, ploty_hi-(plottext/30), label = paste("Trt", env_trt_signif)), size = plottext, hjust = 1)

# check figure
nnnmds

# save as tif
ggsave("alpine_addnuts/figures/journal_figs/nn_nmds.tiff", nnnmds, dpi = 320,
       width = 6, height =3, units = "in")
# save as pdf
ggsave("alpine_addnuts/figures/journal_figs/nn_nmds.pdf", nnnmds, dpi = 320,
       width = 6, height =3, units = "in")
...
```{r plot nutnet 2013 and 2017 together for supplement}

# rename trtcols so M replaces K (K+ in parentheses)
nnnmds_supp <- ggplot(subset(alpplotmds, site == "nutnet" & iter == "suppl"), aes(MDS1, MDS2)) +
  geom_point(aes(fill = trt), size = 1.75, shape = 23, color = "transparent") +
  #geom_polygon(data = sdlhulls, aes(MDS1, MDS2, fill = trt, col = trt), alpha = 0.5) +
  geom_polygon(data=subset(alpells, site == "nutnet" & iter == "suppl"), aes(x=MDS1, y=MDS2,fill=trt), alpha = 0.5) +
  geom_point(data = subset(alpsppmds, site == "nutnet" & iter == "suppl"), aes(MDS1, MDS2, shape = resource_grp, col = resource_grp), alpha = 0.8) +
  scale_fill_manual(name = "Treatment", values = trtcols, labels = gsub("K", "M(K)", names(trtcols))) +
  scale_color_manual(name = "Resource\nstrategy", values = traitcols) +
  scale_shape_manual(name = "Resource\nstrategy", values = c(19, 19, 21)) +
  j_theme +
  # theme(legend.box = "horizontal", 
  #       legend.spacing = unit(0, "pt"),
  #       legend.background = element_blank(),
...
# add FG arrows if any significant
if(any(with(alpenv, pval[site == "nutnet" & grepl("suppl", tracker) & grp == "vectors"]) <= 0.05)){
  nnnmds_supp <- nnnmds_supp + geom_segment(data=subset(alpenv, grp == "vectors" & grepl("suppl", tracker) & site == "nutnet" & pval <= 0.05),aes(x=0,xend=NMDS1,y=0,yend=NMDS2),
                                  arrow = arrow(length = unit(0.25, "cm")),colour="black") + 
    # add label to envfit arrow
    geom_text(data=subset(alpenv, grp == "vectors" & site == "nutnet" & grepl("suppl", tracker) & pval <= 0.05),aes(x=label_x,y=label_y, label = paste0(signif,"F:G")), col = "black", size = plottext, fontface = "italic", hjust = 1)
}

# annotate stress and trt signif
nnnmds_supp <- nnnmds_supp + geom_text(data = subset(alpstress, site == "nutnet" & grepl("suppl", tracker)), aes(plotx_hi, ploty_hi, label = round(stress,3)), size = plottext, hjust = 1) +
  geom_text(data = subset(alpstress, site == "nutnet" & grepl("suppl", tracker)), aes(plotx_hi, ploty_hi-(plottext/29), label = paste("Trt", env_trt_signif)), size = plottext, hjust = 1)

# check figure
nnnmds_supp

# save as tif
ggsave("alpine_addnuts/figures/journal_figs/nn_nmds_supp.tiff", nnnmds_supp, dpi = 320,
       width = 7, height = 3, units = "in")
# save as pdf
ggsave("alpine_addnuts/figures/journal_figs/nn_nmds_supp.pdf", nnnmds_supp, dpi = 320,
       width = 7, height = 3, units = "in")
...
# append enfit trt and ln f:g ratio r2 and pvals
alpenv_stats <- dplyr::select(alpenv, grp, tracker, site, yr, grp, r2, pval) %>% distinct() %>%
  mutate(grp = recode(grp, vectors = "lnF2G", factors = "trt")) %>%
  gather(met, val, r2, pval) %>%
  unite(cat, grp, met, sep = ".") %>%
  mutate(cat = paste("envfit", cat, sep = "."),
         val = round(val, 3)) %>%
  spread(cat, val)

# gather similarity stats anosim R and signif, MRPP delta and signif
alpsim <- data.frame()
for(i in names(alpmds)){
  temp.anosim <- alpmds[[i]][["anosim"]]
  temp.mrpp <- alpmds[[i]][["mrpp"]]
  tempsim <- data.frame(site = gsub("_.*", "", i),
                        tracker = gsub("^[a-z]+_", "", i),
                        yr = parse_number(i),
                        anosim.R = temp.anosim$statistic,
                        anosim.pval = temp.anosim$signif,
                        mrpp.A = temp.mrpp$A,
                        mrpp.pval = temp.mrpp$Pvalue) %>%
    mutate_at(names(.)[grepl("anosim|mrpp", names(.))], function(x) round(x,3))
  alpsim <- rbind(alpsim, tempsim)
}
...
# inititiate list for storing results
indic_df <- data.frame() #df for multiple grps
for(r in grep("main", alpenv_stats$tracker)){
  tempdat <- alpenv_stats[r, c("tracker", "site", "yr")]
  # filter site, yr, and common plots assessed in nmds through time
  tempspp <- filter(abundance_wide, site == tempdat$site, yr == tempdat$yr & plotid %in% c(sdl_common, nn_common)) %>% as.data.frame()
  # create env matrix
  tempenv <- dplyr::select(tempspp, rowid:trt2)
  # set rownames for spp matrix, remove env cols and remove any spp not present in any plots
  rownames(tempspp) <- tempspp$rowid
  tempspp <- tempspp[,!names(tempspp) %in% names(tempenv)] %>% as.data.frame()
  tempspp <- tempspp[,sapply(tempspp, function(x) sum(x) > 0)]
  for(d in c(TRUE, FALSE)){
    tempindic <- multipatt(tempspp, tempenv$trt, func = "IndVal.g", duleg = d, control = how(nperm = 1000))
    # store results 
    tempresults <- data.frame(tempindic$sign, holm.p = p.adjust(tempindic$sign$p.value, method = "holm"), duleg = d) %>%
      rownames_to_column(var = "clean_code2") %>%
      subset(p.value <= 0.1) %>%
      cbind(tempdat)
    # label signif trt
    trtnames <- names(tempresults)[grepl("^s[.][A-Z]", names(tempresults))]
    for(t in trtnames){
      tempresults[t] <- sapply(tempresults[t], function(x) ifelse(x == 1, gsub("^s[.]", "", t), NA))
    }
    #tempresults$trt <- trtcols[tempresults$index]
    # remove trtcols and index col
    #tempresults <- dplyr::select(tempresults, -c(trtcols, "index")) %>%
    tempresults <- unite(tempresults, trts, trtnames, sep = "_") %>%
      mutate(trts = gsub("NA_|_NA", "", trts),
             trts = gsub("_", ", ", trts), 
             trts = trimws(gsub("[.]", "+", trts))) %>%
      # # clean up treatment 
      # mutate(trt = gsub("^s[.]", "", trt),
      #        trt = gsub("[.]", "+", trt)) %>%
      # append spp name and reorder cols
      left_join(distinct(spplist[c("clean_code2", "simple_name")])) %>%
      # join resource acquisition info
      left_join(sppscores[c("clean_code2", "resource_grp")]) %>%
      mutate(resource_grp = ifelse(is.na(resource_grp), "Unknown", resource_grp)) %>%
      dplyr::select(site, yr, duleg, clean_code2, simple_name, resource_grp, trts, stat, p.value, holm.p)
    # append to master df
    indic_df <- rbind(indic_df, tempresults)
  }
}

# clean up
indic_df <- arrange(indic_df, duleg, simple_name, site, yr) %>%
  mutate(site = ifelse(site == "sdl", "Saddle", "NutNet"))

```


```{r plot geum abundance across whole snowfence exp}
#look for moisture sensitivity

alldat <- read.csv("alpine_addnuts/output_data/forTS/sdl_nutnet_fxnl_biodiv_anpp_1997-ongoing.csv")

geum_sffert <- ggplot(subset(alldat, site == "sdl"), aes(as.factor(yr), Geum_rel, col = snow)) +
  #geom_boxplot() +
  #geom_jitter(aes(col = snow), alpha = 0.7, width = 0.2) +
  stat_summary(position = position_dodge(width = 0.2)) +
  labs(y = "Mean relative abundance (%)",
       x = "Year",
       subtitle = "Geum rossii abundance in Saddle snowfence fert exp. by meadow type and snow treatment") +
  facet_grid(trt~meadow)

ggsave("alpine_addnuts/figures/geum_moisturesensitive.pdf", geum_sffert,
       height = 6, width = 6, units = "in", scale = 1.2)

```


```{r}
# write out all multivariate results just in case need to replicate (results can vary slightly (e.g. at hundredths decimal place, some signif if on the border) since results based on permutations)

# saving to my local drive
write_rds(alpmds, "../../Documents/nwt_lter/sdlfert_ms/add_nuts_multivar.rds")
