...
#these should eventually be split and documented
# sourced functions and datasets reference relative paths
# should work if you keep this file in /plotting_scripts'

source ('utility_functions/utility_functions_all.R')
#from https://github.com/gavinsimpson/random_code/blob/master/derivFun.R

#source ('utility_functions/derivFun.R') 
#read in extended summer PCA
extsummer<-readr::read_csv('extended_summer/NWT_sumallPCclimate_19822018.csv')
...
#sometimes it can't sort out what the dates are, so just read and sort out later
datafile1=read.csv(paste0(datadir, names(attributeList)[[1]]), na=c('NA', 'NaN', ''))

##repeat for the other datafile
edi_id='70'
...
#sometimes it can't sort out what the dates are, so just read and sort out later
datafile2=read.csv(paste0(datadir, names(attributeList)[[1]]), na=c('NA', 'NaN', ''))

datafile=dplyr::bind_rows(datafile1, datafile2)

#clean up, remove the other files from memory so your R session won't be so slow
rm(list=c('datafile1', 'datafile2'))

#JEN I UPDATED ONLY UP TO HERE
