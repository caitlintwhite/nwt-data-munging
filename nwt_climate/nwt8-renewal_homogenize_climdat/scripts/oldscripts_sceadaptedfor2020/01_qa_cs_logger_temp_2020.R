...
#original code by CTW 2019
#updated by SCE 2/8/2020 to run for current year's data
...
cr21x <- getTabular(78) %>% as.data.frame()
...
crlogs <- getTabular(405) %>% as.data.frame()
...
crlogs <- crlogs %>%select(LTER_site,local_site,logger,date,year,jday,            
                           airtemp_max,flag_airtemp_max,
airtemp_min,flag_airtemp_min,airtemp_avg,flag_airtemp_avg,
airtemp_hmp1_max,flag_airtemp_hmp1_max,airtemp_hmp1_min,
flag_airtemp_hmp1_min,airtemp_hmp1_avg,flag_airtemp_hmp1_avg,
airtemp_hmp2_max,flag_airtemp_hmp2_max, 
airtemp_hmp2_min,flag_airtemp_hmp2_min,airtemp_hmp2_avg,flag_airtemp_hmp2_avg, 
airtemp_hmp3_max,flag_airtemp_hmp3_max,airtemp_hmp3_min,flag_airtemp_hmp3_min, 
airtemp_hmp3_avg,flag_airtemp_hmp3_avg)

#figure out systematic differences between
#23x/1000 vs hmps

tmps = crlogs %>%
  select(date,  airtemp_max,
         airtemp_min,airtemp_avg,
         airtemp_hmp1_max,airtemp_hmp1_min,
         airtemp_hmp1_avg,
         airtemp_hmp2_max,
         airtemp_hmp2_min,airtemp_hmp2_avg, 
         airtemp_hmp3_max,airtemp_hmp3_min, 
         airtemp_hmp3_avg)%>%
  ...
  mutate(what = case_when(
    ...
    grepl('min', name) ~ 'min',
    grepl('avg', name) ~ 'avg'),
    sensor = ifelse(grepl('hmp', name), 'hmp', 'nothmp')) %>%
  group_by(date, sensor, what) %>%
  summarise(value = mean(value, na.rm = TRUE))%>%
  pivot_wider(id_cols = c(date, what),
              names_from = sensor,
              values_from = value) %>%
  filter(!is.na(hmp)&!is.na(nothmp)) %>%
  mutate(delta = hmp - nothmp)

max_offset = median (tmps%>%filter(what == 'max')%>%pull(delta))
#max_offset -1.553333
min_offset = median (tmps%>%filter(what == 'min')%>%pull(delta))
#min_offset -0.9033333
avg_offset =median (tmps%>%filter(what == 'avg')%>%pull(delta))
#avg_offset -1.10736
avg_offset =median (tmps%>%filter(what == 'avg')%>%pull(delta))


#adjust hmps to avg air 
tmps = crlogs %>%
  select(date,  airtemp_max,
         airtemp_min,airtemp_avg,
         airtemp_hmp1_max,airtemp_hmp1_min,
  ...
  filter(!is.na(hmp))%>%
  select(-nothmp)%>%
  pivot_wider(id_cols = date, names_from = what, values_from = hmp)


#2019-11-08, 2019-11-22 are missing sce check w Jen why this is gone?

crlogs = crlogs %>%
  left_join(., tmps)%>%
  mutate(airtemp_max = ifelse(is.na(airtemp_max), max-max_offset, airtemp_max),
         airtemp_min = ifelse(is.na(airtemp_min), min-min_offset, airtemp_min),
         airtemp_avg = ifelse(is.na(airtemp_avg), avg-avg_offset, airtemp_avg))

dates <- data.frame(date=seq(from = min(crlogs$date), to = max(crlogs$date), by = 'day'))%>%
  mutate(LTER_site = unique (crlogs$LTER_site),
         local_site = unique (crlogs$local_site,
                              logger = "cr1000")) %>%
  mutate(year = lubridate::year (lubridate::ymd(date)),
         jday = lubridate::yday(lubridate::ymd(date)))

crlogs = full_join(crlogs, dates)
...
cr21x <- cr21x %>%
  ...
  dplyr::rename(#airtemp_max = 'maximum temperature',
  ...
  dplyr::select(c(colnames(crlogs), 'time_airtemp_max', 'time_airtemp_min'))
...
cr21x_long <- tidytemp(cr21x, datasource = "cr", special = "time", dropcol = "airtemp_avg") %>%
  mutate(cr_temp = as.numeric(cr_temp), cr_time = as.numeric(cr_time))
...
write_csv(crall_old_new, paste0(datpath, "output_data/prep_data/qa_sdlcr_temp_thru_2019.csv"))
