...
def.plau <- function (
  ...
  for(idxVar in 1:length(data)) {
    ...
    while((idxData <= numData) & (DiffPersMin[idxVar] > 0)) {
      ...
      if(data[idxDataMax,idxVar]-data[idxDataMin,idxVar] >= DiffPersMin[idxVar]) {
        ...
        if(ts[idxData]-ts[idxDataSt] <= TintPers[idxVar]) {
          ...
          # We might have a stuck sensor, but first let's check whether we blew the time threshold b/c # all the data were NA prior to this point
    ...
    if (is.na(data[idxData,idxVar])) {
      ...
      if (ts[idxData]-ts[idxDataSt] > TintPers[idxVar]) {
        # We didn't hit the threshold for the final non-NA points and we were beyond the allowable # time interval for the persistence test, so let's flag the end of the data 
        posFlagPers$fail[[idxVar]] <- unique(c(posFlagPers$fail[[idxVar]],idxDataSt:idxData))
      } else
        # We didn't hit the threshold for the final non-NA points, but we are not yet beyond the # allowable time interval, so let's flag as unable to evaluate
