...
# nothing here so would group grid plots by any factor
...
for(p in unique(veg_only$plot)){
  ...
  temp_df <- data.frame(plot = p, event = unique(plot_df$event), year = unique(plot_df$year))
  temp_df$new_spp <- NA
  ...
  for(i in 2:max(plot_df$event)){
    ...
    print(paste(p, "event", i, "spp gained:", stringr::str_flatten(new_spp, collapse = ", ")))
...
for(p in unique(veg_only$local_site)){
  ...
  temp_df$plot <- p
  temp_df$new_spp <- NA
  spp_list_running <- sort(unique(meta_df$USDA_code[meta_df$event ==1 & meta_df$local_site == p]))
  
  for(i in 2:max(meta_df$event)){
    # grab new native spp gained
    event_spp <- sort(unique(meta_df$USDA_code[meta_df$event == i & meta_df$local_site == p]))
    new_spp <- event_spp[!event_spp %in% spp_list_running]
...
with(spp_gains_meta_names_df, lapply(split(spp_gains, plot), function(x) x[duplicated(x)])) #only NAs duplicated, which is fine (some yrs didn't lose any spp)
# only NAs, good
...
for(p in unique(veg_only$local_site)){
  ...
  temp_df$plot <- p
  temp_df$lost_spp <- NA
  spplist_lag1 <- unique(meta_df$USDA_code[meta_df$event ==1 & meta_df$local_site == p])
  ...
  while(i %in% 2:max(meta_df$event)){
    ...
    # grab native spp list for current sampling event
    event_spp <- unique(meta_df$USDA_code[meta_df$event == i & meta_df$local_site == p])
    # compare spp that disappeared from year prior
    event_spp_loss <- spplist_lag1[!spplist_lag1 %in% event_spp]
    # compare spp lost from year prior to future species list
    event_spp_loss <- event_spp_loss[!event_spp_loss %in% fwd_spplist]
...
# check to be sure no codes duplicated
with(lost_spp_meta_names_df, lapply(split(lost_spp,plot), function(x) x[duplicated(x)])) #only NAs duplicated, which is fine (some yrs didn't lose any spp)
